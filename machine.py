...
class MACHINE():
    ...
    def valid_move(self):
        return [[point1, point2] for (point1, point2) in list(combinations(self.whole_points, 2)) if
        ...
        if (len(point1_connected) >= 2 and len(point2_connected) >= 2):
            for line1, line2 in product(point1_connected, point2_connected):
                if ((line1[0] in line2 or line1[1] in line2) and (line2[0] in line1 or line2[1] in line1)):
                    ...
                    if ((self.check_pointIntri(point1, point2, point0)) and (
                    self.check_pointIntri(point1, point2, point00))):
                        return True
        return False
